...
def Model(history_step=30, predict_step=15):
    model = Sequential()
    model.add(Dense(100, activation='relu', input_dim=history_step))
    ...
    for history_id in range(len(history_class)):
        ...
        for predict_id in range(len(predict_class)):
            ...
            for user_id in range(1, user_train_size + 1):
                ...
                y_validate = y_validate.reshape(y_validate.shape[0], y_validate.shape[1])
            ...
            model.fit(x_train, y_train, batch_size=config['batch_size'], epochs=config['epochs'], validation_data=(x_validate, y_validate))
